...
func TestUpstreamUpdatedTime(t *testing.T) {
	server := NewWebconfigServer(sc, true)
	server.SetUpstreamEnabled(true)
	router := server.GetRouter(true)
	cpeMac := util.GenerateRandomCpeMac()
	...
	// ==== step 9 verify all states deployed ====
	subdocUpdatedTimeMap := make(map[string]string)
	for _, subdocId := range subdocIds {
		url = fmt.Sprintf("/api/v1/device/%v/document/%v", cpeMac, subdocId)
		req, err = http.NewRequest("GET", url, nil)
		req.Header.Set("Content-Type", "application/msgpack")
		...
		updatedTimeText := res.Header.Get(common.HeaderSubdocumentUpdatedTime)
		if len(updatedTimeText) > 0 {
			subdocUpdatedTimeMap[subdocId] = updatedTimeText
		}
	}
...
func TestUpstreamResponseSkipDbUpdate(t *testing.T) {
	server := NewWebconfigServer(sc, true)
	server.SetUpstreamEnabled(true)
	router := server.GetRouter(true)
	cpeMac := util.GenerateRandomCpeMac()
	...
	assert.NilError(t, err)
	assert.Equal(t, len(mpartMap), 1)
	_, ok = mpartMap["portforwarding"]
	assert.Assert(t, ok)

	// ==== step 11 verify all states deployed ====
	// srcbytesMap changed
	expectedStateMap := map[string]int{
		"privatessid": common.Deployed,
		"lan":         common.Deployed,
		"wan":         common.Deployed,
	}
	for _, subdocId := range subdocIds {
...
func TestUpstreamResponseSkipDbUpdateNone(t *testing.T) {
	server := NewWebconfigServer(sc, true)
	server.SetUpstreamEnabled(true)
	router := server.GetRouter(true)
	cpeMac := util.GenerateRandomCpeMac()
	...
	// ==== step 10 GET /config with schemaVersion change to trigger upstream ====
	configUrl = configUrl + "?group_id=root"
	req, err = http.NewRequest("GET", configUrl, nil)
	assert.NilError(t, err)
